...
import {
  StyleSheet, Text, View, Dimensions, TouchableOpacity, ScrollView,
...
class Moment extends Component {
    ...
    if (this.props.cache.moment !== null && this.props.cache.moment.id === this.props.id) {
      ...
      fetch(`${apiUrl}/moment/read?id=${this.props.id}`, {
        ...
        .then((moment) => {
          const like = [];
          moment[2].forEach((d) => {
            like.push(parseInt(d.user_id, 10));
          });
          let data;
          if (moment[3].length === 5) {
            data = {
              data: moment[0], like, list: moment[3], refresh: false
            };
            this.setState(data);
          } else {
            data = {
              data: moment[0], like, list: moment[3], load: false, refresh: false
            };
            this.setState(data);
          }
  ...
  clickLike() {
    ...
    const action = this.state.like.indexOf(this.props.userId) === -1 ? 1 : 0;
  ...
  loadMore() {
    fetch(`${apiUrl}/moment/load?id=${this.props.id}&load=${this.state.load - 1}&add=${this.state.send}`, {
      ...
      .then((result) => {
        const add = this.state.list.concat(result);
        if (result.length === 10) {
          ...
          this.props.userId ? (
            <View style={styles.rootContain}>
              ...
              <TextInput
                ...
                multiline
                numberOfLines={5}
                onChangeText={text => this.setState({ comment: text.substr(0, 150) })
                }
