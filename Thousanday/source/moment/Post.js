...
class PostMoment extends Component {
  ...
  // send moment
  sendMoment() {
    const content = this.state.info.trim();
    ...
    if (!pet) {
      ...
      type = type.split('/')[1];
      type = `.${type}`;
      const file = { uri: image.uri, type: 'multipart/form-data', name: type };
      ...
      fetch(`${apiUrl}/upload/moment`, {
        ...
        .then((result) => {
          ...
          this.props.goMoment(parseInt(result[0], 10));
        });
    }
  }
  // choose a pet
  ...
  render() {
    ...
    const pets = this.state.petList.map(pet => (
      <TouchableOpacity
        key={`choosepet${pet.pet_id}`}
        style={
          this.state.pet === pet.pet_id ? styles.petChoose : null
        }
    ...
    return (
      ...
      >
        ...
        <TextInput
          ...
          multiline
          numberOfLines={4}
          onChangeText={text => this.setState({ info: text.substr(0, 120) })
          }
